...
function berg_list_fields_book($action, $bookId, $typebook) {	//print "Action : <b>$action</b> et bookId = $bookId<br/><br/>";		global $ficBook;	global $ficTypesBook;	global $icorpuspic_tab;		//Nombre maximum de personnes qui numérisent	$max_numerisation_persons = 4;		//Chargement du fichier de book	$b=simplexml_load_file($icorpuspic_tab.$ficBook);		//Chargement du fichier de typeofbook	$t=simplexml_load_file($icorpuspic_tab.$ficTypesBook);		if($action == "mod")		$title = "Edit a book";	else		$title = "Add a book";		//Si on est en mode ajout, on cherche le book à modifier et sinon on détermine le nouveau  bookId	foreach($b as $boo){		if($boo->id == $bookId){			$book = $boo;		}		$newbookId = $boo->id;	}	$newbookId+=1;		print "	<center><h3>$title</h3>
	<form action=\"\" method=\"post\">	<input type=\"hidden\" name=\"action\" value=\"$action\" />	<table>";		//bookId -> Si on est en mode ajout, le bookId est calculé automatiquement	print "<tr>
		...
		<td><input type=\"text\" name=\"id\" size=\"3\" value=\"";		if($action=="mod") print $book->id;	else print $newbookId;		print "\" disabled /></td>
	   </tr>";	//book type	print "<tr>
	...
	print "<td>Person ".($i+1)." : <br/><input type=\"text\" name=\"numerisation_person".($i+1)."\" size=\"40\" value=\"";		if($action=="mod") print utf8_decode($book->numerisation['person'.($i+1)]);		print "\" /></td>
		...
		<td>Word format : <br/><input type=\"file\" name=\"format_doc\" value=\"";		if($action=="mod") print utf8_decode($book->format['doc']);		print "\" /></td>
		</tr>		<tr>
		<td>Acrobat format : <br/><input type=\"file\" name=\"format_pdf\" value=\"";		if($action=="mod") print utf8_decode($book->format['pdf']);		print "\" /></td>
		</tr>\n";		print "</table><br/><input type=\"submit\" value=\"";		if($action=="add") 		print "add the book";	else		print "edit the book";		print "\" /></form>\n";		//Bouton de retour	print "<form action=\"book.php\" name=\"retour\" method=\"post\">			<td>				<input type=\"hidden\" name=\"action\" value=\"view\" />				<input type=\"hidden\" name=\"typebook\" value=\"$typebook\" />				<input type=\"submit\" value=\"back\" />			</td>	</form>	</center>\n";	}function berg_book_del($bookId, $typebook) {	echo "<center>			<strong>Confirmer la suppression du book n°$bookId ?</strong>			<br/><br/>			<table border=\"0\">			<tr><form action=\"book.php\" name=\"confirmation\" method=\"post\">			<td>					<input type=\"hidden\" name=\"supp\" value=\"del\" />					<input type=\"hidden\" name=\"action\" value=\"view\" />					<input type=\"hidden\" name=\"typebook\" value=\"$typebook\" />					<input type=\"hidden\" name=\"bookId\" value=\"$bookId\" />					<input type=\"submit\" value=\"Valider\" />					</td></form>			<form action=\"book.php\" name=\"annulation\" method=\"post\">			<td>				<input type=\"hidden\" name=\"action\" value=\"view\" />				<input type=\"hidden\" name=\"typebook\" value=\"$typebook\" />				<input type=\"submit\" value=\"Annuler\" />			</td>			</form></tr>			</table>		</center>";}		/*	foreach($b->book[0]->children() as $name => $node) {
		echo "$name : $node\n";		if($node->attributes()!= ""){						foreach($node->attributes() as $attribute => $value) {
			   echo "<b>$attribute</b>=", $value, "\n<br>";
			}		}		echo "<br>";		//if($name == "")
	}				foreach($b->children() as $name => $node) {		$book[$name] = $node;	}		print "<pre>";		print_r($book);		print "</pre>";	*//*
function berg_list_fields_book_mod($bookId,$typebook) {	echo "Modification : typebook=$typebook et bookid=$bookId";
	// mod book	/*
	if ($action == "mod") {
	print "
	<form action=\"book_mod.php\" method=\"post\">
	<table>";
	$sql = "SELECT * FROM ".$table."_book WHERE bookId = '$_POST[bookId]'";
	$result = mysql_query($sql) or die ("erroe");
		while($val = mysql_fetch_array($result)) {
			$sqlf = "SHOW FIELDS FROM ".$table."_book";
			$resultf = mysql_query($sqlf);
			while($valf = mysql_fetch_array($resultf)) {
			$field_name = htmlspecialchars($valf['Field']);
				if ($field_name == "bookId" ) {
				print "<tr>
						<td align=\"right\" valign=\"top\">$field_name :</td>
						<td><input type=\"text\" name=\"$field_name\" value=\"$val[$field_name]\" size=\"10\"></td>
					   </tr>";
				}
				elseif ($field_name == "typeofbookId" ) {
				print "<tr>
						<td align=\"right\" valign=\"top\">$field_name :</td>
						<td><input type=\"text\" name=\"$field_name\" value=\"$val[$field_name]\" size=\"10\"></textarea></td>
					   </tr>";
				} else {
				print "<tr>
						<td align=\"right\" valign=\"top\">$field_name :</td>
						<td><textarea cols=\"60\" rows=\"10\" name=\"$field_name\">$val[$field_name]</textarea></td>
					   </tr>";
				}
			}
