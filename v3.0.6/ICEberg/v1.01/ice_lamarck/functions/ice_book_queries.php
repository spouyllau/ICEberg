...
$result = mysql_query($sql);
	while ($val = mysql_fetch_array($result)) {
		if ($lang == "fr") {
			print "<td>
			<a href=\"$_SERVER[PHP_SELF]?lang=$_GET[lang]&type=$_GET[type]&bdd=$_GET[bdd]&table=$_GET[table]&typeofbookId=$val[typeofbookId]&num=0\">[$val[typeofbookDes]]</a>
			</td>";
		}
		if ($lang == "en") {
			print "<td>
			<a href=\"$_SERVER[PHP_SELF]?lang=$_GET[lang]&type=$_GET[type]&bdd=$_GET[bdd]&table=$_GET[table]&typeofbookId=$val[typeofbookId]&num=0\">[$val[typeofbookDesGb]]</a>
			</td>";
		}
	}
...
// affichage
	while (($val = mysql_fetch_array($result)) && ($Ncur<$Nmax+$Ndeb)) {
	  if($Ncur>=$Ndeb) {
		print "
		<li>$val[bookDate], <a href=\"ice_book_detail.php?lang=$_GET[lang]&type=$_GET[type]&bdd=$_GET[bdd]&table=$_GET[table]&bookId=$val[bookId]&typeofbookId=$val[typeofbookId]&num=$_GET[num]\" id=\"$val[bookId]\"><b>$val[bookShortTitle]</b></a></li>
		";
	  } $Ncur++;
	}
...
if ($lang == "fr") {
	...
	<td>$valbook[bookTitle]</td>
	</tr>
	<tr> 
	<td width=\"20%\">Auteur</td>
	<td>$valbook[bookAuthorName] $valbook[bookAuthorSurname]</td>
	</tr>";
	if ($valbook[bookPagesNumber]) { 
		print "<tr>
		<td>Format</td><td>$valbook[bookPagesNumber]</td>
	</tr>"; 
	}
	print "
	<tr>
	<td width=\"20%\">Editeur</td>
	<td>$valbook[bookEditor] $valbook[bookPlaceEditor]</td>
	</tr>
	<tr> 
	<td width=\"20%\">Date</td>
	<td>$valbook[bookDate]</td>
	</tr>
	<tr>
	<td width=\"20%\">Observations</td>
	<td>$valbook[bookObservations]</td>
	</tr>
	";
	if (($valbook[bookDoc]) OR ($valbook[bookPdf])) {
	print "
	<tr>
	<td width=\"20%\">Document en texte intégral téléchargeable</td>
	<td>";
	}
...
if ($lang == "en") {
	...
	if ($valbook[bookPagesNumber]) { 
		print "<tr>
		<td width=\"20%\">Page(s)</td>
		<td>$valbook[bookPagesNumber]</td>
	</tr>"; 
	}
	print "
	<tr>
	<td width=\"20%\">Editor</td>
	<td>$valbook[bookEditor] $valbook[bookPlaceEditor]</td>
	</tr>
	<tr> 
	<td width=\"20%\">Date</td>
	...
	<tr>
	<td width=\"20%\">Digitalized document</td>
	<td>
	"; 
	if ($valbook[bookDoc]){
		print "<a href=\"$valbook[bookUrl]$valbook[bookDoc]\" lang=\"fr\">
...
$result = mysql_query($sql);
	while ($valpage = mysql_fetch_array($result)) {
			if ($valpage['pageChapter']) {
				print "
				<tr>
					<td><a href=\"ice_page_detail.php?lang=$_GET[lang]&type=$_GET[type]&bdd=$_GET[bdd]&table=$_GET[table]&typeofbookDes=$typeofbookDes&bookId=$valpage[bookId]&pageChapter=$valpage[pageChapter]&pageOrder=$valpage[pageOrder]&facsimile=off&num=$_GET[num]&nav=1\">$valpage[pageChapter]</a></td>
					<td>$valpage[pageNumber]</td>
				</tr>";
			}
	}
}
