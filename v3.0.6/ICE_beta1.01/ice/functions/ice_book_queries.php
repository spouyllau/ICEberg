...
$result = mysql_query($sql);
	while ($valpage = mysql_fetch_array($result)) {
			if ($valpage['pageChapter']) {
				print "
				<tr>
					<td class=\"tdi\"><a href=\"ice_page_detail.php?lang=$_GET[lang]&type=$_GET[type]&bdd=$_GET[bdd]&table=$_GET[table]&typeofbookDes=$typeofbookDes&bookId=$valpage[bookId]&pageChapter=$valpage[pageChapter]&pageOrder=$valpage[pageOrder]&facsimile=off&num=$_GET[num]&nav=1\">$valpage[pageChapter]</a></td>
